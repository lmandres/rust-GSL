...
fn call_multifit_df(x: *const gsl_vector, data: *mut multifit_nlinear_data, jacobian: *mut gsl_matrix) -> i32 {
...
fn call_multifit_fvv(x: *const gsl_vector, v: *const gsl_vector, params: *mut multifit_nlinear_data, fvv: *mut gsl_vector) -> i32 {
    GSL_SUCCESS
}
...
fn run_gsl_multifit_nlinear_df(
    ...
    let call_multifit_df_ptr = if func_dfs.len() == params_len {
        call_multifit_df as *const fn(*const gsl_vector, *mut multifit_nlinear_data, *mut gsl_matrix) -> i32
    } else {
        ptr::null()
    };
...
#[repr(C)]
#[derive(Debug, Clone)]
struct multilarge_nlinear_data {
  func_f: fn(Vec<f64>, f64, Vec<f64>) -> f64,
  func_dfs: Vec<fn(Vec<f64>, f64, Vec<f64>) -> f64>,
  params_len: usize,
  ts: Vec<f64>,
...
pub struct gsl_multilarge_nlinear_parameters {
    trs: *const gsl_multilarge_nlinear_trs,
    scale: *const gsl_multilarge_nlinear_scale,
    solver: *const gsl_multilarge_nlinear_solver,
...
pub struct gsl_multilarge_nlinear_fdf {
  f: fn(*const gsl_vector, *mut multilarge_nlinear_data, *mut gsl_vector) -> i32,
  df: *const fn(*const gsl_vector, *mut multilarge_nlinear_data, *mut gsl_matrix) -> i32,
  fvv: *const fn(*const gsl_vector, *const gsl_vector, *mut multilarge_nlinear_data, *mut gsl_vector) -> i32,
  n: usize,
  p: usize,
  params: *mut multilarge_nlinear_data,
  nevalf: usize,
  nevaldfu: usize,
  nevaldf2: usize,
  nevalfvv: usize
...
extern "C" {
    pub fn gsl_multilarge_nlinear_default_parameters() -> self::gsl_multilarge_nlinear_parameters;
}
extern "C" {
    pub fn gsl_multilarge_nlinear_alloc(
        T: *const gsl_multilarge_nlinear_type,
        params: *const self::gsl_multilarge_nlinear_parameters,
        n: usize,
        p: usize,
...
fn call_multilarge_df(x: *const gsl_vector, data: *mut multilarge_nlinear_data, jacobian: *mut gsl_matrix) -> i32 {
...
fn call_multilarge_fvv(x: *const gsl_vector, v: *const gsl_vector, params: *mut multilarge_nlinear_data, fvv: *mut gsl_vector) -> i32 {
    GSL_SUCCESS
}

pub unsafe fn gsl_multilarge_nlinear_basic(
    func_f: fn(Vec<f64>, f64, Vec<f64>) -> f64,
    params_in: Vec<f64>,
    ts: &Vec<f64>,
    ys: &Vec<f64>,
    ...
    gsl_multilarge_nlinear_basic_df(
        func_f,
        &Vec::new(),
        params_in,
        ts,
...
pub unsafe fn gsl_multilarge_nlinear_basic_df(
    func_f: fn(Vec<f64>, f64, Vec<f64>) -> f64,
    func_dfs: &Vec<fn(Vec<f64>, f64, Vec<f64>) -> f64>,
    params_in: Vec<f64>,
    ts: &Vec<f64>,
...
fn run_gsl_multilarge_nlinear_df(
    func_f: fn(Vec<f64>, f64, Vec<f64>) -> f64,
    func_dfs: &Vec<fn(Vec<f64>, f64, Vec<f64>) -> f64>,
    params: *mut f64,
    parerr: *mut f64,
    ...
    let trust: *const gsl_multilarge_nlinear_type = unsafe { gsl_multilarge_nlinear_trust };
    let fdf_params: self::gsl_multilarge_nlinear_parameters = unsafe { self::gsl_multilarge_nlinear_default_parameters() };

    let f: *mut gsl_vector;
    let jacobian: *mut gsl_matrix;
    let covar: *mut gsl_matrix = unsafe { gsl_matrix_alloc(params_len, params_len) };

    let mut data_struct = multilarge_nlinear_data {
        func_f: func_f,
        func_dfs: func_dfs.to_vec(),
        params_len: params_len,
        ts: ts.to_vec(),
    ...
    let data_ptr: *mut multilarge_nlinear_data = &mut data_struct as *mut multilarge_nlinear_data;
    let x = unsafe { gsl_vector_view_array(params, params_len) };
    let x_ptr: *const gsl_vector = &x.vector as *const gsl_vector;
    let dof: f64 = (vars_len - params_len) as f64;
    ...
    let call_multilarge_df_ptr = if func_dfs.len() == params_len {
        call_multilarge_df as *const fn(*const gsl_vector, *mut multilarge_nlinear_data, *mut gsl_matrix) -> i32
    } else {
        ptr::null()
    };

    let mut fdf = self::gsl_multilarge_nlinear_fdf {
        f: call_multilarge_f,
        df: call_multilarge_df_ptr,
        fvv: ptr::null(),
        n: vars_len,
        p: params_len,
        params: data_ptr,
    ...
    let fdf_ptr: *mut self::gsl_multilarge_nlinear_fdf = &mut fdf as *mut self::gsl_multilarge_nlinear_fdf;

    /* allocate workspace with default parameters */
    let w: *mut gsl_multilarge_nlinear_workspace = unsafe {
        self::gsl_multilarge_nlinear_alloc(
            trust,
            &fdf_params as *const self::gsl_multilarge_nlinear_parameters,
            vars_len,
            params_len
        )
    };
    ...
    for i in 0..max_iters {
        ...
        unsafe { gsl_multilarge_nlinear_iterate(w); }
        unsafe { gsl_multilarge_nlinear_test(xtol, gtol, ftol, &mut *status, w); }
        ...
        unsafe { gsl_multilarge_nlinear_rcond(&mut rcond, w); }
        if rcond.is_nan() && i != 0 {
            println!("Reason for stopping: Invalid Status");
            *status = -1;
    ...
    /* compute covariance of best fit parameters */
     unsafe { gsl_multilarge_nlinear_covar(covar, w); }

    /* compute final cost */
    let residuals = unsafe { gsl_multilarge_nlinear_residual(w) };
    unsafe { gsl_blas_ddot(residuals, residuals, &mut chisq); }
    let chisq_dof = (chisq / dof).sqrt();

    let x_gsl_vec: *const gsl_vector = unsafe { gsl_multilarge_nlinear_position(w) };
    for i in 0..params_len {
        unsafe { params.wrapping_add(i).write(gsl_vector_get(x_gsl_vec, i)); }
        unsafe { parerr.wrapping_add(i).write(chisq_dof * gsl_matrix_get(covar, i, i).sqrt()); }
    }

    unsafe { gsl_multilarge_nlinear_free(w); }
    unsafe { gsl_matrix_free(covar); }
}
